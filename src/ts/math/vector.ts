...
function vector2AngleAndDistance([x1, y1]: Vector2, [x2, y2]: Vector2) {
  const dx = x2 - x1;
  const dy = y2 - y1;
  return [Math.atan2(dy, dx), Math.sqrt(dx*dx + dy*dy)];
}

function vector2Rotate(a: number, [x, y, z]: Vector2): Vector2 {
...
function vectorNRound<T extends number[]>(v: T): T {
  return v.map(v => Math.round(v)) as any;
}

function vector3TransformMatrix4(m: Matrix4, x: number, y: number, z: number): Vector3 {
